...
class GameCommand(commands.Cog):
    ...
    async def _startNewGame(self, ctx):
        ...
        """
        # Check webhook service for this channel, webhooks can send multiple embed object at once in a single message
        # This requires manage-webhook permission
        """
        usingWebhook = ''
        webhooklist = await ctx.message.channel.webhooks()  
        for this_webhook in webhooklist:                # Lets look for the webhook list for this channel
            if this_webhook.name == '[犯罪現場-CS File] Webhook':
                usingWebhook = this_webhook             # Use the found webhook
        if usingWebhook == '':                          # No existing webhook found, lets create a new one
            with open("icon.png", "rb") as image:       # A webhook icon only accept bytes like object as input, we convert a png into a bytes object first
                f = image.read()
            usingWebhook = await ctx.message.channel.create_webhook(name = '[犯罪現場-CS File] Webhook', reason = 'Allowed for multiple images message.', avatar = f)
        """
        self.playerlist = []
        self.player_rolelist = []
        self.volunteerlist = []
        self.gamecardtuple = []
        self.dict = {}
        self.dict['defaultMessage'] = ctx.message # For accessing the guild and channel info
        ...
        self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist'].append(thisplayer)
        embed = discord.Embed(title=f'User `{ctx.message.author.display_name}` wanted to start a game of [CS File].', description=f'Type in the command **[joinGame]** to join the event.\nIf all the users are ready. Type in the command **[gameContinue]** to start the game.', colour=int(jdata['Embed_Theme_Colour'],16)) #,color=Hex code
        embed.add_field(name=f'用戶 `{ctx.message.author.display_name}` 新建了一個 [犯罪現場] 的活動請求', value=f'請輸入 **[joinGame]** 以加入遊戲\n當所有玩家都加入後，請輸入**[gameContinue]**指令繼續。')
    ...
    async def _joinGame(self, ctx):
        ...
        """
        try:
            localPL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist']
            find_player = self.checkUserinIdList(ctx.message.author, localPL, ctx)
            if find_player in localPL:   # If the user has not joined yet, you can comment out this 2 line to test the game easier
                self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist'].remove(find_player)
            self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist'].append(find_player)   # Add the message sender into the player list
            embed = discord.Embed(title=f'User `{ctx.message.author.display_name}` joined the [CS File] game.', description=f'There are currently {len(localPL)} users joined the game.', colour=int(jdata['Embed_Theme_Colour'],16))
            embed.add_field(name=f'用戶 `{ctx.message.author.display_name}` 已經加入 [犯罪現場] ', value=f'現在已經有 {len(localPL)} 名玩家加入了')
            embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
            await ctx.send(embed=embed)
        except:
    ...
    async def _leaveGame(self, ctx):
        ...
        try:
            localPL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist']
            localVL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['volunteerlist']
            find_player = self.checkUserinIdList(ctx.message.author, localPL, ctx)
            ...
            embed = discord.Embed(title=f'User `{ctx.message.author.display_name}` left the [CS File] game.', description=f'There are currently {len(localPL)} users in the game.', colour=int(jdata['Embed_Theme_Colour'],16)) 
            embed.add_field(name=f'用戶 `{ctx.message.author.display_name}` 已經退出 [犯罪現場] ', value=f'現在剩下 {len(localPL)} 名玩家在遊戲中')
            embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
            await ctx.send(embed=embed)
        except:
            await self.askForCommand(ctx)
    ...
    async def _playerList(self, ctx):
        ...
        for person in localPL:
            playerlist_string += "> " + str(player_counter) + '. ' + person.player.display_name + "\n"
            player_counter += 1
        embed = discord.Embed(title=f'Joined players\n已加入的玩家', description=f'{playerlist_string}', colour=int(jdata['Embed_Theme_Colour'],16)) 
        embed.add_field(name='--------', value=f'Total of {len(localPL)} players joined.\n合共有 {len(localPL)} 名玩家。')
        embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
        await ctx.send(embed=embed)
        #except:
            #await self.askForCommand(ctx)
    ...
    async def _volunteerFS(self, ctx):
        ...
        try:
            localPL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist']
            localVL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['volunteerlist']
            find_player = self.checkUserinIdList(ctx.message.author, self.volunteerlist, ctx)
            if(find_player not in localPL):
    ...
    async def _leaveVolunteer(self, ctx):
        ...
        try:
            localVL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['volunteerlist']
            find_player = self.checkUserinIdList(ctx.message.author, localVL, ctx)
            if(find_player in localVL):
                localVL.remove(find_player)
            else:
                return
            embed = discord.Embed(title=f'You left the volunteer list.', description=f'You are no longer a volunteer for forensic scientist.', colour=int(jdata['Embed_Theme_Colour'],16)) 
    ...
    async def _volunteerList(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(title=f'Volunteers for forensic scientist. \n鑑鉦專家志願者', description=f'{volunteerlist_string}', colour=int(jdata['Embed_Theme_Colour'],16)) 
            embed.add_field(name='--------', value=f'Total of {len(localVL)} volunteers.\n合共有 {len(localVL)} 名志願者。')
            embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
            await ctx.send(embed=embed)
        except:
            await self.askForCommand(ctx)
    ...
    async def _gameContinue(self, ctx):
        ...
        localPRL.append(Roles.ForensicScientist(ctx, lucky_winner.player, 'Forensic Scientist')) 
        ...
        if(len(ori_playerlist) >= 6 and self.enable_witness == True):    
            ...
            localPRL.append(Roles.Witness(ctx, lucky_witness.player, 'Witness'))
            localPL.remove(lucky_witness)
        # The others are all normal Investigator
        for person in localPL:   
            localPRL.append(Roles.Investigator(ctx, person.player, 'Investigator'))
        localPL = ori_playerlist.copy()
        random.shuffle(localPRL) # Shuffle the list, this affects the order to presentations.
        del ori_playerlist
        
        embed = discord.Embed(title=f'Identity messages has beed sent though all players private message.', description=f'所有玩家的身份都已經由私訊發出。', colour=int(jdata['Embed_Theme_Colour'],16))
        embed.add_field(name=f'The Forensic Scientist please wait for murderer to pick the evidence.', value=f'請鑑證專家等待兇手選擇現埸證據。', inline=True)
    ...
    async def _showAllCard(self, ctx, toPrivate = 'False'):
        ...
        player_rolelist = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['player_rolelist']
        try:
            for person in player_rolelist:
                await self._showCard(ctx, person.player, str(toPrivate))   # Send all player's card to the public channel
        except :
            await self.askForCommand(ctx)
    ...
    async def _showCard(self, ctx, sp_user : discord.Member , toPrivate : str = 'True'):
        ...
        try:
            if sp_user == None: return # The sp_user does not exist in this server
        ...
        if(person.identityIs('Forensic Scientist') or person.identityIs('Player')):
            ...
            #       > name_tw
            #       > url
            embed = discord.Embed(title=f'[{person.getPlayerName()}] Clues 線索', description=f'----------', colour=int(jdata['Embed_Theme_Colour'],16))
            field_count = 0
            for each_clue in person.getClueList():
                embed.add_field(name=f'_{each_clue[en]}_', value=f'***{each_clue[tw]}***', inline=True)
                field_count += 1
                if(field_count % 2 == 0):
                    embed.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
            embed.add_field(name = f'[{person.getPlayerName()}] Methods of Operation 犯案手法', value = f'----------', inline = False)
            field_count = 0
            for each_method in person.getMethodList():
                embed.add_field(name=f'_{each_method[en]}_', value=f'***{each_method[tw]}***', inline=True)
                field_count += 1
                if(field_count % 2 == 0):
                    embed.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
            embed.set_image(url = cardBGurl)
            embed.set_footer(text=f"犯罪現場-CS File  --@{ctx.guild.name}.{ctx.message.channel}") # maybe add an icon later
    ...
    async def _gameRules(self, ctx, Page = 1, toPrivate = True):
        try:
            ...
            p = 1
        embed = discord.Embed(title=f'[犯罪現場-CS File] - Rules 規則說明', description=f'----------', colour=int(jdata['Embed_Theme_Colour'],16))
        embed.set_thumbnail(url = jdata['Icon'])
        embed.set_footer(text=f"犯罪現場-CS File     P.{p}") # maybe add an icon later
        if p == 1:
    ...
    async def _drawRandomScenes(self, ctx):
        ...
        try:
            localPRL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['player_rolelist']
            ForensicScientist : Roles.ForensicScientist = self.findIdentity(localPRL, 'Forensic Scientist')
    ...
    async def _drawEvents(self, ctx):   
        ...
        if ctx.message.author == ForensicScientist.player:
            ...
            embed=discord.Embed(title=f"The event tile you drew is `{draw_event[en]}`.", description=f"你抽到的事件卡是 `{draw_event[tw]}`", color=int(jdata['hintCard_Colour'],16))
            embed.add_field(name='\a', value=f'{draw_event[c_en]}', inline = False)
            embed.add_field(name='\a', value=f'{draw_event[c_tw]}', inline = False)
            embed.set_footer(text=f'犯罪現場-CS File  @{ctx.guild.name}.{ctx.message.channel.name}')
    ...
    async def _testStart(self, ctx):
        ...
        for i in range(0,7):    # Tester play as all the character
            self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['playerlist'].append(Roles.Identity(ctx, ctx.message.author, 'Player'))
    ...
    async def _forceJoin(self, ctx, args):
        ...
        try:
            ...
            if find_user != None:
                if(find_user.bot == True): return
                thisplayer = self.checkUserinIdList(find_user, localPL, ctx)
                if(thisplayer in localPL):   # If the user has not joined yet, you can comment out this 2 line to test the game easier
                    localPL.remove(thisplayer)
                localPL.append(thisplayer)   # Add the message sender into the player list
                embed = discord.Embed(title=f'User `{find_user.display_name}` joined the [CS File] game.', description=f'There are currently {len(localPL)} users joined the game.', colour=int(jdata['Embed_Theme_Colour'],16))
                embed.add_field(name=f'用戶 `{find_user.display_name}` 已經加入 [犯罪現場] ', value=f'現在已經有 {len(localPL)} 名玩家加入了')
                embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
                await ctx.send(embed=embed)
                return
    ...
    async def sendIdentityMessage(self, ctx):
        ...
        for person in player_rolelist:
            # Send message  
            if(person.guildId == ctx.guild.id and person.textChannelId == ctx.message.channel): # To make sure not to mistaken the data from other channel or servers
                if(person.in_murderer_team):
                    embed = discord.Embed(title=f'You have been choosen as [{person.identity}]', description=f'{person.roles_guide[en]}', colour=int(jdata['Murderer_Team_Colour'],16))
                    embed.add_field(name=f'你已經被選中成為 [{person.identity_tw}]', value=f'{person.roles_guide[tw]}', inline=True)
                    embed.set_footer(text=f"犯罪現場-CS File @{ctx.guild.name}.{ctx.message.channel.name}") # maybe add an icon later
                    embed.set_image(url = person.url)
                    await person.player.send(embed = embed)
                else:   # In investigator team
                    embed = discord.Embed(title=f'You have been choosen as [{person.identity}]', description=f'{person.roles_guide[en]}', colour=int(jdata['Investigator_Team_Colour'],16))
                    embed.add_field(name=f'你已經被選中成為 [{person.identity_tw}]', value=f'{person.roles_guide[tw]}', inline=True)
                    embed.set_footer(text=f"犯罪現場-CS File @{ctx.guild.name}.{ctx.message.channel.name}") # maybe add an icon later
                    embed.set_image(url = person.url)
                    await person.player.send(embed = embed)
    ...
    async def distributeCard(self, ctx):
        ...
        for person in player_rolelist:
            if(person.guildId == ctx.guild.id and person.textChannelId == ctx.message.channel):
                if(person.identityIs('Forensic Scientist')): continue
                else:
                    for i in range(0, self.clue_count):   # Distribute clue_count clues to a player
                        choose_clue = random.choice(clues_list) # increase the randomness
                        person.addClue(choose_clue)
                        clues_list.remove(choose_clue)
                    for i in range(0, self.method_count): # Distribute method_count methods to a player
                        choose_method = random.choice(method_list) # increase the randomness
    ...
    async def sendFSCard(self, ctx, cardtype : str = 'locCard', page : str = '1', modify_message : discord.message = None, toPrivate = True):
        ...
        if modify_message != None:
            ...
            local_PRL = self.guildDict[str(MM_dl[0].id)][str(MM_dl[1].id)]['player_rolelist']
            guildname = MM_dl[0].name
            channelname = MM_dl[1].name
        else:
            local_PRL = self.guildDict[str(ctx.guild.id)][str(ctx.message.channel.id)]['player_rolelist']
        ...
        if(cardtype == 'locCard'):  # The card is 'location of crime'
            ...
            for every_location in self.UtilObj.getlocCard()[page]:
                message_string += "> " + emojis[num_counter] + "   " + every_location + "\n"
                num_counter += 1
        ...
        embed = discord.Embed(title=f'Please select the [{string_en}] \n(Click the referred emoji)', description=f'請選擇 [{string_tw}] (按下相應的表情符號)', colour=int(jdata[str(cardtype+'_Colour')],16)) #,color=Hex code
        embed.add_field(name=f"@{guildname}.{channelname}\n-------",value=f"{message_string}")
        embed.set_image(url = image)
        embed.set_footer(text=footer) # maybe add an icon later

        # Modify the message if it is called by on_reaction_add event
        FS_player : Roles.ForensicScientist = self.findIdentity(local_PRL, 'Forensic Scientist')
        ...
        if(modify_message != None and ctx == ''):
            ...
            new_info_tuple = (ctx.guild, ctx.message.channel, cardtype, int(page))
    ...
    async def sendMurdererCard(self, ctx, sendClues = True, sendMethods = True):
        ...
        # Embed message build
        embed=discord.Embed(title="Please Select 1 'Clue' as the evidence.", description="請選擇 '一種線索' 作為證物。", color=int(jdata['Murderer_Team_Colour'],16))
        for each_clue in Murderer.getClueList():
            embed.add_field(name=f'{emojis[field_count]} _{each_clue[en]}_', value=f'***{each_clue[tw]}***', inline=True)
            field_count += 1
            if(field_count % 2 == 0):
                embed.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
        embed.add_field(name = f'Click the emojis below to select the clue.', value = f'點擊下方的表情符號以選擇證物。', inline = False)
        embed.set_footer(text=f'犯罪現場-CS File  @{ctx.guild.name}.{ctx.message.channel.name}')
        embed.set_image(url = CardData.CluesList().get_singleBGurl())
        embed.set_thumbnail(url = jdata['Evil_face'])

        field_count = 0
        embed2=discord.Embed(title="Please Select 1 'Method of Operation' as the evidence.", description="請選擇 '一種作案工具' 作為證物。", color=int(jdata['Murderer_Team_Colour'],16))
        for each_method in Murderer.getMethodList():
            embed2.add_field(name=f'{emojis[field_count]} _{each_method[en]}_', value=f'***{each_method[tw]}***', inline=True)
            field_count += 1
            if(field_count % 2 == 0):
                embed2.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
        embed2.add_field(name = f'Click the emojis below to select the method of operation.', value = f'點擊下方的表情符號以選擇作案工具。', inline = False)
        embed2.set_footer(text=f'犯罪現場-CS File  @{ctx.guild.name}.{ctx.message.channel.name}')
        ...
        if sendClues: 
            ...
            self.dmchannellist[str(MD_message1.id)] = new_info_tuple    # Add the message into the local memoery dictionary for storage
        if sendMethods: 
            ...
            self.dmchannellist[str(MD_message2.id)] = new_info_tuple    # Add the message into the local memoery dictionary for storage
    ...
    async def murdererSelectClue(self, MM_message, clue_number : int, confirmed = False):
        ...
        localPRL = self.guildDict[str(localDM[0].id)][str(localDM[1].id)]['player_rolelist']
        Murderer : Roles.Murderer = self.findIdentity(localPRL, 'Murderer')
        if not confirmed: 
            emojis = ['❌', '✅']
            new_info_tuple = (localDM[0], localDM[1], 'MD_M1_V', clue_number)
            selected_clue = Murderer.getClueList()[clue_number - 1] # This is the card picked
        else:
            emojis = []
        ...
        # Embed message build
        embed=discord.Embed(title=f"You have picked ` {selected_clue[en]} ` as the clue evidence.", description=f"你選擇了 ` {selected_clue[tw]} ` 作為線索證物。", color=int(jdata['Murderer_Team_Colour'],16))
        for each_clue in Murderer.getClueList():
            ...
            if(field_count % 2 == 0):
                embed.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
        if not confirmed: embed.add_field(name = f'Click the emojis below to confirm or cancel the selection.', value = f'點擊下方的表情符號以確定或重新選擇。', inline = False)
        embed.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
    ...
    async def murdererSelectMethod(self, MM_message, method_number : int, confirmed = False):
        ...
        if not confirmed: 
            ...
            selected_method = Murderer.getMethodList()[method_number - 1] # This is the card picked
        else:
            emojis = []
        ...
        # Embed message build
        embed=discord.Embed(title=f"You have picked ` {selected_method[en]} ` as the method evidence.", description=f"你選擇了 ` {selected_method[tw]} ` 作為作案證物。", color=int(jdata['Murderer_Team_Colour'],16))
        for each_method in Murderer.getMethodList():
    ...
    async def fSselectLoc(self, message : discord.Message, select_loc_num : int, confirmed = False):
        ...
        if not confirmed: 
            emojis = ['❌', '✅']
            new_info_tuple = (localDM[0], localDM[1], 'locCard_V', locpage, select_loc_num)
            selected_loc = LCardlist[str(locpage)][select_loc_num-1]
        else:
            emojis = []
            importance = 0  # Not implemented yet
            new_info_tuple = (localDM[0], localDM[1], 'locCard_C', localDM[3], localDM[4])
            selected_loc = LCardlist[str(localDM[3])][localDM[4]-1]
            ForensicScientist.locInfo = ( selected_loc, importance, selected_loc_url, locpage )
        ...
        for location in LCardlist[str(locpage)]:
            if location == selected_loc:
                ...
                embed_field_string += ">  " + " ❌  " + location + "\n"
        embed=discord.Embed(title=f"You have picked ` {selected_loc} ` as the location of crime.", description=f"你選擇了 ` {selected_loc} ` 作為案發現場。", color=int(jdata['locCard_Colour'],16))
    ...
    async def fSselectCod(self, message : discord.Message, select_cod_num : int, confirmed = False):
        ...
        if not confirmed: 
            emojis = ['❌', '✅']
            new_info_tuple = (localDM[0], localDM[1], 'codCard_V', select_cod_num)
        ...
        for reasons in codCard['content']:
            if reasons == selected_cod:
                ...
                embed_field_string += ">  " + " ❌  " + reasons + "\n"
        embed=discord.Embed(title=f"You have picked ` {selected_cod} ` as the {codCard[en]}.", description=f"你選擇了 ` {selected_cod} ` 作為{codCard[tw]}。", color=int(jdata['codCard_Colour'],16))
        embed.add_field(name='\a', value=f'{embed_field_string}')
        if not confirmed: embed.add_field(name = f'Click the emojis below to confirm or cancel the selection.', value = f'點擊下方的表情符號以確定或重新選擇。', inline = False)
        embed.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
    ...
    async def fSselectHint(self, message : discord.Message, select_hint_num : int, confirmed = False):
        ...
        if not confirmed: 
            emojis = ['❌', '✅']
            new_info_tuple = (localDM[0], localDM[1], 'hintCard_V', hintpage, select_hint_num)
            select_hint = HCard['content'][select_hint_num-1]
        else:
            emojis = []
            importance = 0  # Not implemented yet
            new_info_tuple = (localDM[0], localDM[1], 'hintCard_C', localDM[3], localDM[4])
            select_hint = HCard['content'][localDM[4]-1]
            hintInfo_tuple =  ( select_hint, importance, select_hint_url, hintpage )
        ...
        for options in HCard['content']:
            if options == select_hint:
                ...
                embed_field_string += ">  " + " ❌  " + options + "\n"
        embed=discord.Embed(title=f"You have picked ` {select_hint} ` for {HCard[en]}.", description=f"你為 {HCard[tw]} 選擇了  ` {select_hint} ` 。", color=int(jdata['hintCard_Colour'],16))
        embed.add_field(name='\a', value=f'{embed_field_string}')
        if not confirmed: embed.add_field(name = f'Click the emojis below to confirm or cancel the selection.', value = f'點擊下方的表情符號以確定或重新選擇。', inline = False)
        embed.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
    ...
    async def notifyAccompliceEvidence(self, message):
        ...
        localPRL = self.guildDict[str(localDM[0].id)][str(localDM[1].id)]['player_rolelist']
        Murderer : Roles.Murderer = self.findIdentity(localPRL, 'Murderer')
        Accomplice : Roles.Accomplice = self.findIdentity(localPRL, 'Accomplice')
        ...
        if Murderer.chosenClue != None and Murderer.chosenWeapon != None:
            ...
            # Embed message build
            embed=discord.Embed(title=f"The murderer has picked ` {selected_clue[en]} ` as the clue evidence.", description=f"兇手選擇了 ` {selected_clue[tw]} ` 作為線索證物。", color=int(jdata['Murderer_Team_Colour'],16))
            for each_clue in Murderer.getClueList():
                if each_clue[en] == selected_clue[en]:
                    embed.add_field(name=f'✅ _{each_clue[en]}_', value=f'***{each_clue[tw]}***', inline=True)
                else:
            ...
            # Embed message build
            embed2=discord.Embed(title=f"The murderer has picked ` {selected_method[en]} ` as the method evidence.", description=f"兇手選擇了 ` {selected_method[tw]} ` 作為作案證物。", color=int(jdata['Murderer_Team_Colour'],16))
            for each_method in Murderer.getMethodList():
                if each_method[en] == selected_method[en]:
                    embed2.add_field(name=f'✅ _{each_method[en]}_', value=f'***{each_method[tw]}***', inline=True)
                else:
                    embed2.add_field(name=f'_{each_method[en]}_', value=f'***{each_method[tw]}***', inline=True)
                field_count += 1
                if(field_count % 2 == 0):
                    embed2.add_field(name='\a', value='\a', inline=True)    # Line break, work as <br>
            embed2.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
    ...
    async def notifyFSEvidence(self, message, ctx = None, revealToPublic = False):
        ...
        tw = "name_tw"
        field_count = 0
        if not revealToPublic: localDM = self.dmchannellist[str(message.id)]
        else: localDM = [ctx.guild, ctx.message.channel]
        localPRL = self.guildDict[str(localDM[0].id)][str(localDM[1].id)]['player_rolelist']
        Murderer : Roles.Murderer = self.findIdentity(localPRL, 'Murderer')
        ForensicScientist : Roles.ForensicScientist = self.findIdentity(localPRL, 'Forensic Scientist')
        selected_clue = Murderer.chosenClue
        ...
        if Murderer.chosenClue != None and Murderer.chosenWeapon != None:
            # Embed message build
            embed=discord.Embed(title=f"The murderer has picked ` {selected_clue[en]} ` as the clue evidence.", description=f"兇手選擇了 ` {selected_clue[tw]} ` 作為線索證物。", color=int(jdata['Investigator_Team_Colour'],16))
            for each_clue in Murderer.getClueList():
                if each_clue[en] == selected_clue[en]:
                    embed.add_field(name=f'✅ _{each_clue[en]}_', value=f'***{each_clue[tw]}***', inline=True)
                else:
            ...
            # Embed message build
            embed2=discord.Embed(title=f"The murderer has picked ` {selected_method[en]} ` as the method evidence.", description=f"兇手選擇了 ` {selected_method[tw]} ` 作為作案證物。", color=int(jdata['Investigator_Team_Colour'],16))
            for each_method in Murderer.getMethodList():
                if each_method[en] == selected_method[en]:
                    embed2.add_field(name=f'✅ _{each_method[en]}_', value=f'***{each_method[tw]}***', inline=True)
                else:
    ...
    async def notifyAllFShints(self, message):
        ...
        for location in LCardlist[str(locpage)]:
            if location == selected_loc[0]:
                ...
                embed_field_string += ">  " + " ❌  " + location + "\n"
        embed=discord.Embed(title=f"The Forensic Scientist thinks that ` {selected_loc[0]} ` is the location of crime.", description=f"鑑證專家認為案發地點位於 ` {selected_loc[0]} ` 。", color=int(jdata['locCard_Colour'],16))
        embed.add_field(name='\a', value=f'{embed_field_string}')
        embed.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
        ...
        embed2=discord.Embed(title=f"The Forensic Scientist thinks that ` {selected_cod[0]} ` is the cause of death.", description=f"鑑證專家認為死因是 ` {selected_cod[0]} ` 。", color=int(jdata['codCard_Colour'],16))
        ...
        for every_hint_tuple in selected_hintlist:
            ...
            for hc in HCard['content']:
                if hc == hint:
                    ...
                    embed_field_string += ">  " + " ❌  " + hc + "\n"
            hintembed=discord.Embed(title=f"The Forensic Scientist thinks the {HCard[en]} is ` { hint } `.", description=f"鑑證專家認為{HCard[tw]}是 ` { hint } ` 。", color=int(jdata['hintCard_Colour'],16))
            hintembed.add_field(name='\a', value=f'{embed_field_string}')
            hintembed.set_footer(text=f'犯罪現場-CS File  @{localDM[0].name}.{localDM[1].name}')
    ...
    async def askForCommand(self, ctx, needed_command='startNewGame'):
        ...
        embed = discord.Embed(title=f'Command not working.', description=f'Type in the command **[{needed_command}]** before you use this command.', colour=int(jdata['Embed_Theme_Colour'],16)) #,color=Hex code
        embed.add_field(name=f'指令運行失敗。', value=f'請先輸入 **[{needed_command}]** 指令後，再輸入該指令。', inline=True)
        embed.set_footer(text="犯罪現場-CS File") # maybe add an icon later
        await ctx.send(embed=embed)
    ...
    async def getDefaultCtx(self, message):
        ...
        defaultMessage = self.guildDict[str(locDML[0].id)][str(locDML[1].id)]['defaultMessage']
    ...
    def checkUserinIdList(self, user, Idlist, ctx = '', returnBool = False):
        ...
        else: return Roles.Identity(ctx, user, 'Player')
    ...
    def findIdentity(self, rolelist, looking_identity : str = ''):
        ...
        for IdClass in rolelist:
            if IdClass.identity == looking_identity: return IdClass
